...
func (methodPicker) A(ctx context.Context) (string, error) { return "A", nil }
func (methodPicker) B(ctx context.Context) (string, error) { return "B", nil }

func TestHelpersTellMethodValuesApart(t *testing.T) {
	// A method value compiles to a wrapper the runtime reports as <autogenerated>:1, the same for every
	// method. Two different methods are two different declarations, so they must be two identities.
	var p methodPicker
	r, err := futura.NewFlowFromContainer[struct{}, string](containertest.NewInMemory()).Execute(t.Context(), func(b futura.FlowBuilder, _ struct{}) (string, error) {
		useA := futura.State(b, true)
		pick := p.B
		if useA.V() {
			pick = p.A
		}
		v, err := futura.Source(b, pick)
		if err != nil {
			return "", err
		}
		if useA.V() {
	...
	assert.NoError(t, err)
	assert.Equal(t, "B", r, "the memo of method A was returned for method B")
}
